<template>
	...
	<view class="container">
		<view class="padding-tb-xl" style="background-color: #FD6416;"></view>
		...
		<view class="user-section">
			...
			<view class="vip-card-box"
				style="background: rgba(0,0,0,.7); margin-left:52upx auto;width: 700upx;position: absolute;top:240upx;left:26upx">
				...
				<view class="tit" v-if="!relation_id">
					...
					<text v-if="isEnable != '否'">会员领红包享省钱+返现</text>
					<text v-if="isEnable == '否'">省钱兄外卖省钱专家</text>
				</view>
		...
		<view class="orders" v-if="isEnable != '否'">
			...
			<view class="tui-box tui-tool-box" style="margin:0 auto;">
				...
				<view class="tui-order-list tui-flex-wrap" style="height:160rpx;">
					...
					<view v-if="isEnable != '否'" class="tui-tool-item tti" @click="navToLogins('/pages/order/team')">
						<view class="tui-icon-box">
				...
				<view class="tui-order-list tui-flex-wrap goods_flex">
					...
					<view class="number">
						...
						<view style="font-size:24upx;color:#666666;margin-top:18upx">今日订单
							<text style="color: #FD6416;margin-left: 16upx;">{{orderNum}}笔</text>
						</view>
					</view>
				</view>
		...
		<view style="width: 100%;" class="tools">
			...
			<view class="tui-box tui-tool-box" style="margin:0 auto;">
				...
				<view class="tui-order-list tui-flex-wrap">
					...
					<view class=" tui-tool-item" @click="navToLogins('/pages/member/account')">
						<view class="tui-icon-box">
	...
	export default {
		...
		onShow() {
			...
			if (image_url && image_url !== 'undefined') {
				...
				this.image_url = '/static/img/logo.png';
			}
			if (gender) {
				this.gender = gender;
			}
			...
			if (userId) {
				this.getMoney();
				this.getUserInfo(userId);
				this.$Request.getT('/taobao/getRelationIdByUserId?userId=' + userId).then(res => {
					if (res.status == -200) {} else if (res.status == 0) {
						this.getUserInfo(userId);
					}
				});
		...
		methods: {
				...
				if (!token) {
					...
					if (!relation_id) {
						...
						that.$Request.getT('/taobao/getRelationIdByUserId?userId=' + userId).then(res => {
							if (res.status == -200) {
								this.$Request.getT('/taobao/getTwd?userId=' + userId).then(ress => {
									// let tkl = JSON.parse(ress.data).tbk_tpwd_create_response.data.model
									let tkl = '';
									
									this.$Request.getT('/common/type/173').then(res => {
										if (res.status == 0) {
											if (res.data && res.data.value) {
												this.shouquanType=res.data.value
												if(res.data.value==1){
													
													//获取口令
													this.getTKL()
												}else{
													this.$Request.getT('/common/type/8').then(ress => {
														if (ress.status == 0) {
															if (ress.data && ress.data.value) {
																...
																tkl = ress.data.value+'?uid=' + userId + '&token=' + token 
																uni.setClipboardData({
																	data: tkl,
																	success: (res) => {
																		this.isOpen = true
																		uni.showModal({
																			title: '复制成功',
																			content: '口令已经复制成功,请打开浏览器登录淘宝账号访问进行授权！',
																			showCancel: false,
																			cancelText: '',
																			confirmText: '我知道了',
																			success: res => {
			...
			getTKL() {
				...
				this.$Request.getT('/taobao/getTwd?userId=' + userId).then(res => {
					let tkl = JSON.parse(res.data).tbk_tpwd_create_response.data.model
					uni.setClipboardData({
						data: tkl,
						success: (res) => {
							this.isOpen = true
...
<style lang="scss" scoped>
	...
	.tixian {
		background: -moz-linear-gradient(left, #FD6416, #FD6416 100%);
		background: -webkit-gradient(linear, left top, left right, color-stop(0, #FD6416), color-stop(100%, #FD6416));
