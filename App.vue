<script>
	...
	export default {
		...
		onShow() {
			...
			let that = this;
			//获取全局是否开启佣金功能
			that.$Request.getT('/common/type/106').then(res => {
				if (res.status == 0) {
			...
			that.$Request.getT('/common/type/172').then(res => {
				if (res.status == 0) {
					if (res.data && res.data.value) {
						this.$queue.setData('checkModel', res.data.value);
					}
				}
			});
			//获取列表是否开启佣金功能
			that.$Request.getT('/common/type/95').then(res => {
				if (res.status == 0) {
					if (res.data && res.data.value) {
						this.$queue.setData('isLieBiaoEnable', res.data.value);
					}
				}
			});
			//最高佣金 41
		...
		onLaunch: function() {
			...
			that.$Request.getT('/common/type/92').then(res => {
				if (res.status == 0) {
					if (res.data && res.data.value) {
						this.$queue.setData('threeMoney', res.data.value);
					}
				}
			});
			//团队佣金
			...
			//是否打开发卡模式
			that.$Request.getT('/common/type/100').then(res => {
				if (res.status == 0) {
					if (res.data && res.data.value) {
						this.$queue.setData('isFaKa', res.data.value);
					}
				}
			});
			//是否打开微信支付
			that.$Request.getT('/common/type/101').then(res => {
				if (res.status == 0) {
					if (res.data && res.data.value) {
						this.$queue.setData('isWeiXinPay', res.data.value);
					}
				}
			});
			//是否打开支付宝支付
			that.$Request.getT('/common/type/102').then(res => {
				if (res.status == 0) {
					if (res.data && res.data.value) {
						this.$queue.setData('isAliPay', res.data.value);
					}
				}
			});
		}
